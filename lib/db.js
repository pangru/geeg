...
var db = module.exports = (function () {
  ...
  var notice = function (count, callback) {
    console.log('%j', conf);
    var q = "SELECT nid, title, body, name, create_date, read_cnt FROM %s INNER JOIN %s ON writer_email = email ORDER BY nid DESC";
    if (count) 
      q += "  LIMIT " + count;
  ...
  var calendar = function (count, callback) {
    var q = "SELECT eid, title, edate, place, create_date, writer_email FROM %s INNER JOIN %s ON writer_email = email ORDER BY eid DESC";
    if (count) {
      q += "  LIMIT " + count;
    }
